...
class ReportController extends Controller {
  ...
  public function getSalesReport() {
    $model = \Solunes\Business\App\Sale::where('status','!=','holding');
    $array = \Business::check_report_header($model);
    $array['show_place'] = true;

    $codes_array = ['income_sale', 'income_sale_credit', 'expense_refund'];
    $account_array = \Solunes\Business\App\Account::whereIn('code', $codes_array)->lists('id')->toArray();
    $accounts = \Solunes\Business\App\PlaceAccountability::whereIn('account_id', $account_array)->where('created_at', '>=', $array['i_date'])->where('created_at', '<=', $array['e_date']);
    if($array['place']!='all'){
    ...
    foreach($accounts as $item){
      $new_total = \Business::calculate_currency($item->amount, $array['currency'], $item->currency);
      if($item->account->code=='expense_refund'){
        ...
        foreach($item->other_accounts as $other){
          $other_amount = \Business::calculate_currency($other->real_amount, $array['currency'], $currency);
    ...
    $array = $array + ['total'=>$sales_total, 'business'=>$business, 'cash'=>$cash,'pos'=>$pos, 'web'=>$web, 'online'=>$online, 'pending'=>$pending_total, 'refund_total'=>$refund_total];
    // Gráficos
    $type_items = [['type'=>'paid','total'=>round($business)], ['type'=>'web','total'=>round($web)], ['type'=>'online','total'=>round($online)], ['type'=>'pending','total'=>round($pending_total)]];
    ...
    return \Business::check_report_view('business::list.sales-report', $array);
  }
