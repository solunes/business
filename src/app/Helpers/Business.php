...
class Business {
    ...
    public static function get_cart() {
        if($cart = \Solunes\Business\App\Cart::checkOwner()->checkCart()->status('holding')->with('cart_items','cart_items.product')->first()){
          $cart->touch();
        } else {
          $cart = new \Solunes\Business\App\Cart;
          if(\Auth::check()){
    ...
    public static function add_cart_item($cart, $product, $quantity) {
        if($cart_item = $cart->cart_items->where('product_id', $product->id)->first()){
          ...
          $cart_item = new \Solunes\Business\App\CartItem;
    ...
    public static function check_category_children($category, $category_array = []) {
        ...
        if(count($category->children)>0){
          foreach($category->children as $subcategory){
            $category_array = \Business::check_category_children($subcategory, $category_array);
          }
        }
    ...
    public static function generate_code($type) {
        if($last = \Solunes\Business\App\TransactionCode::orderBy('code', 'DESC')->first()){
            $last_code = $last->code;
        } else {
            $last_code = 0;
        }
        $new = $last_code + 1;
        $new_code = new \Solunes\Business\App\TransactionCode;
        $new_code->type = $type;
        $new_code->code = $new;
        $new_code->save();
        return $new;
    ...
    public static function register_sale_payment($sale, $payment_id, $currency_id, $status, $amount, $detail, $exchange = 1) {
        $sale_payment = new \Solunes\Business\App\SalePayment;
    ...
    public static function register_account_array($array, $created_at = NULL, $transaction_code = NULL) {
        if(!$transaction_code){
            $transaction_code = \Business::generate_code('auto');
        }
        if(!$created_at){
        ...
        foreach($array as $item){
            if($item['amount']>0){
                $account_detail = new \Solunes\Business\App\PlaceAccountability;
    ...
    public static function inventory_movement($place, $product, $type, $quantity, $name, $transaction, $item, $transaction_code = NULL) {
        ...
        $product_movement = new \Solunes\Business\App\InventoryMovement;
        ...
        if($product_stock = \Solunes\Business\App\ProductStock::where('parent_id', $product->id)->where('place_id', $place->id)->first()){
            $product_stock->quantity = $product_stock->quantity + $real_quantity;
        } else {
            $product_stock = new \Solunes\Business\App\ProductStock;
        ...
        if($transaction!='register_product_purchase'){
            ...
            $asset_stock = \Solunes\Business\App\Account::getCode('asset_stock')->id;
            if($transaction=='register_product_drop'){
                $expense_sale = \Solunes\Business\App\Account::getCode('expense_inventory_loss')->id;
            } else {
                $expense_sale = \Solunes\Business\App\Account::getCode('expense_sale')->id;
            }
            if($type=='move_out'){
                $stock_type = 'credit';
                $expense_type = 'debit';
            ...
            $arr[] = \Business::register_account($place->id, $stock_type, $asset_stock, $currency_id, $amount, $name);
            $arr[] = \Business::register_account($place->id, $expense_type, $expense_sale, $currency_id, $amount, $name);
            \Business::register_account_array($arr, $item->created_at, $transaction_code);
        }
    }
    ...
    public static function check_report_header($model, $places = [], $extra = NULL) {
        ...
        $currencies = \Solunes\Business\App\Currency::where('in_accounts', 1)->get()->lists('real_name', 'id');
        ...
        if($extra&&$extra=='account_id'){
            ...
            $array['accounts'] = \Solunes\Business\App\Account::lists('name','id');
            $array['current_account_id'] = 1;
            if(request()->has('account_id')){
                $array['current_account_id'] = request()->input('account_id');
            }
        }
        $array['places'] = ['all'=>'Consolidado']  + \Solunes\Business\App\Place::lists('name', 'id')->toArray() + $places;
    ...
    public static function calculate_shipping_cost($shipping_id, $city_id, $weight) {
        $shipping = \Solunes\Business\App\Shipping::find($shipping_id);
